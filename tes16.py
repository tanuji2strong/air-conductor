...
WINDOW_NAME = "4/4 Conductor (Down-Left-Right-Up)"
...
def draw_guidance_arrow(img, direction):
    """Draw a big arrow indicating the expected direction for the next beat."""
    h, w, _ = img.shape
    center_x = w // 2
    center_y = h // 2

    length = min(w, h) // 3  # arrow length
    thickness = 8
    color = (0, 255, 255)    # yellow
    tipLength = 0.15

    if direction == "down":
        start = (center_x, center_y - length // 2)
        end   = (center_x, center_y + length // 2)
    elif direction == "up":
        start = (center_x, center_y + length // 2)
        end   = (center_x, center_y - length // 2)
    elif direction == "left":
...
with mp_hands.Hands(
    ...
    # you can keep a fixed size, fullscreen will scale it
    w, h = 960, 540

    global prev_x, prev_y, last_dir, dir_frames, beat_count, beat_in_bar
    ...
    while True:
            ...
            if prev_x is not None and prev_y is not None:
                ...
                if current_dir == "none":
                    ...
                    if current_dir == expected_dir and dir_frames == DIR_HOLD_FRAMES:
                        ...
                        # avoid repeated triggers on same stroke
        ...
        # draw guidance arrow for next beat
        expected_dir = PATTERN[beat_in_bar]
        draw_guidance_arrow(img, expected_dir)

        # HUD text
        cv2.putText(img, f"Beats total: {beat_count}", (30, 60),
                    fontFace, 1.0, (255, 255, 255), 2, lineType)
        cv2.putText(img, f"BPM: {int(bpm)}", (30, 100),
                    fontFace, 1.0, (0, 255, 0), 2, lineType)
        cv2.putText(img, f"Beat in bar: {beat_in_bar} (expect: {expected_dir})",
                    (30, 140), fontFace, 0.9, (255, 255, 0), 2, lineType)
        cv2.putText(img, f"Current dir: {current_dir}",
                    (30, 180), fontFace, 0.9, (0, 255, 255), 2, lineType)
