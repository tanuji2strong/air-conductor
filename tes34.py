...
# --- CONFIG ---
COUNTDOWN_SECONDS = 3
RADIUS = 80             # radius (pixels) for target zones
MIN_INTERVAL = 0.25     # min seconds between beats
MAX_INTERVAL = 3.0      # max seconds between beats

USE_RIGHT_ARM = True    # True: use right arm (12,14,16,20,22); False: left
...
# NOTE: pattern is flipped horizontally:
...
def draw_pattern_and_targets(img, cx, cy, active_beat):
    ...
    Pattern is horizontally flipped: Down -> RIGHT -> LEFT -> Up.
    """
    L = 120
    base_color = (60, 60, 60)
    ...
    # ACTIVE stroke (flipped: beat 2 = right, beat 3 = left)
    if active_beat == 1:
    ...
    # Labels (2 on right, 3 on left to match flipped pattern)
    cv2.putText(img, "1", (bottom[0] + 10, bottom[1]), font, 0.6, (150,150,150), 1)
    cv2.putText(img, "3", (left[0] - 10, left[1] - 10), font, 0.6, (150,150,150), 1)
    cv2.putText(img, "2", (right[0] - 10, right[1] - 10), font, 0.6, (150,150,150), 1)
...
def run_countdown():
    ...
    while True:
        ...
        cv2.putText(frame, text, (cx - tw // 2, cy + th // 2),
                    font, 3, color, 6)
        cv2.putText(frame, "Right arm: Down -> RIGHT -> LEFT -> Up",
    ...
    while cap.isOpened():
        ...
        if results.pose_landmarks:
            ...
            if interval_ok:
                ...
                # Beat 2: RIGHT (flipped)
                elif expected_beat == 2 and dist(fingertip, right_pt) < RADIUS:
                    beat_count += 1
                    flash_timer = 5
                    if last_beat_time is not None:
                        bpm = 60.0 / (now - last_beat_time)
                ...
                # Beat 3: LEFT (flipped)
                elif expected_beat == 3 and dist(fingertip, left_pt) < RADIUS:
                    beat_count += 1
                    flash_timer = 5
                    if last_beat_time is not None:
                        bpm = 60.0 / (now - last_beat_time)
